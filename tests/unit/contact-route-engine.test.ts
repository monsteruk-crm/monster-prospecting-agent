...
describe("deriveContactRoutes", () => {
  test("keeps a role-only route when no contact page is evidenced", () => {
    const routes = deriveContactRoutes({
      ...
      classification: { primaryCategory: "FAMILY_ATTRACTION_OPERATOR" as const, secondaryCategories: ["LEISURE_ENTERTAINMENT_GROUP"], buyerModel: "OWNER_OPERATOR" as const }, relevanceHypothesis: "An attraction.", discoveredSignals: [], possibleBuyerRoles: ["Head of Programming"], discoveryEvidenceIds: [`source:${"a".repeat(64)}`], unresolvedQuestions: [],
  ...
  test("falls back to mission buyer roles when extraction returns none", () => {
    const routes = deriveContactRoutes({
      accountKey: "example.org:example", companyName: "Example", officialDomain: "https://example.org", website: "https://example.org/visit",
      classification: { primaryCategory: "FAMILY_ATTRACTION_OPERATOR" as const, secondaryCategories: [], buyerModel: "OWNER_OPERATOR" as const }, relevanceHypothesis: "An attraction.", discoveredSignals: [], possibleBuyerRoles: [], discoveryEvidenceIds: [], unresolvedQuestions: [],
    }, [], ["Commercial Director"]);
  ...
  test("keeps a publicly confirmed email on the route and supports email-only filtering", () => {
    const account = {
      ...
      classification: { primaryCategory: "FAMILY_ATTRACTION_OPERATOR" as const, secondaryCategories: [], buyerModel: "OWNER_OPERATOR" as const }, relevanceHypothesis: "An attraction.", discoveredSignals: [], possibleBuyerRoles: ["Partnerships Director"], discoveryEvidenceIds: [`source:${"b".repeat(64)}`], unresolvedQuestions: [],
    };
