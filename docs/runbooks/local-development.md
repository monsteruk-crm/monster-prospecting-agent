...
The PostgreSQL `sslmode` warning is not itself a mission failure. The database client normalises legacy `prefer`, `require` and `verify-ca` modes to explicit `verify-full`; the deployment should still keep its provider’s current connection string in the configured environment variable. DuckDuckGo searches use the HTML endpoint’s form `POST` surface because GET requests can be rejected with HTTP 403 by hosted-function egress addresses. A 403, 202 challenge response, or non-parseable result page remains a visible provider error and is never bypassed: the live stream reports the failed query and provider message, while the server emits an error-level structured event (`mission.discovery.search_provider_failed`) plus the request/run ID. If all provider queries fail, the run can complete with zero accounts and the persisted dossier shows the exact partial-research errors.
