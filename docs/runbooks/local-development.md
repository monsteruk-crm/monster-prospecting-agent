...
The New mission builder launches through `POST /api/missions/discover/stream`, and the run screen shows saved stage/query events while the graph runs. Every run ID is written to PostgreSQL. The `/runs` page loads `GET /api/runs?limit=20`; opening a row calls `GET /api/runs/:missionRunId`. Use `Search deeper` on a completed dossier to call `POST /api/runs/:missionRunId/search-more`; it adds bounded budget, deduplicates previous URLs and attaches new query/search progress to the same run.
