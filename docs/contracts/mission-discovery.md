...
`contactRequirement` defaults to `ANY_ROUTE`. When it is `PUBLIC_EMAIL`, when a buyer-role phrase such as `any available email` is used, or when mission instructions clearly say to return only contacts with an email, the discovery graph reserves up to one focused same-site search and three official contact-page fetches per candidate account before market search/source fetching. It checks retained official-page links and contact metadata, then uses the reserved budget for targeted enrichment. If no deterministic email is found, one bounded extraction-model call may parse the fetched official excerpt; its email and supporting quote must both be present in that excerpt. A syntactically valid email must still be explicitly present in official-source content; it is stored on the provenance-linked contact route and exported, never inferred. Accounts without a qualifying public email remain in the audit dossier with `CONTACT_REQUIREMENT_NOT_MET` and are excluded from the email-required export.

When `requiredSignals` and `preferredSignals` are both empty, the preparation graph omits the signal-discovery query family and the verification stage has no signal model work to perform. It does not substitute default signals.
