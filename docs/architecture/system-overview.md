...
The bounded discovery segment consumes prepared mission state through `discoverSalesMission`: `search_provider` calls the configured Brave Search API adapter when its subscription token is present, or the DuckDuckGo browser-like HTML adapter with bounded Bing fallback when no Brave key is configured; an injected `SearchProvider` remains available for tests and controlled alternatives. `fetch_official_sources` calls `safe_fetchTool` within the page budget, `extract_accounts` uses the registry's extraction model on bounded untrusted excerpts, `verify_buying_signals` uses the registry's verification model on typed candidates, and the post-canonicalisation contact phase uses official-page metadata plus bounded same-site contact discovery before deterministic route and score recalculation. Search results, contact metadata and model outputs are schema-validated; source failures and model failures become typed partial-result errors and do not invalidate successful accounts. `POST /api/missions/discover` remains the synchronous JSON boundary for API compatibility; the New mission builder and run screen use `POST /api/missions/discover/stream` for durable live stage/query output. Executed queries and cumulative search results are bound to the `SalesMissionRun` through its mission ID/run ID and audit events; raw page bodies are never streamed. Route failures emit structured server logs with a request correlation ID.
...
Bootstrap non-goals are vector indexing, broad contact crawling, automatic outreach, Redis, queues, browser automation and live CRM writes. The bounded discovery graph now uses Brave Search API for search when configured, `safe_fetchTool` for source acquisition, registry-selected structured extraction and verification models for bounded interpretation, Prisma persistence for mission evidence and pending review snapshots, Postgres checkpoint/resume with review decisions, provenance-linked contact routes, approved first-move drafts and approved CSV dry-run export.
