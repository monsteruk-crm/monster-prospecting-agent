...
Postgres stores missions, runs, accounts, evidence, buying signals, review snapshots and audit events. Persistence uses stable mission/run/entity keys and Prisma upserts; review snapshots store references and bounded state, not duplicate full source pages. LangGraph remains the sole workflow state machine; a durable Postgres checkpoint/resume adapter is still a later slice. Codex native memory is development continuity only and is not part of the Monster Scout runtime.

Bootstrap non-goals are contact discovery, scoring, human-in-the-loop graph interrupts, pgvector ingestion, automatic outreach, Redis, queues, browser automation and CRM integration. The bounded discovery graph now uses DuckDuckGo for search, `safe_fetchTool` for source acquisition, registry-selected structured extraction and verification models for bounded interpretation, and Prisma persistence for mission evidence and pending review snapshots; durable graph resume, review decisions and human interrupts remain unimplemented.
