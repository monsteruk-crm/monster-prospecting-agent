...
- Prefer linked official pages, then at most one focused same-site DuckDuckGo form-POST search per account, then two deterministic contact paths. Fetch at most three contact pages per account within the global mission page budget.
...
- Keeping DuckDuckGo GET: rejected because hosted-function requests can receive HTTP 403 while the bounded form-POST surface returns the HTML results page.
