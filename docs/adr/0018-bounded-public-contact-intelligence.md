...
- Treat malformed or concatenated visible email text as a contact-hint parsing issue: repair a recognised public suffix where safe, otherwise discard only that hint. A malformed email candidate must not fail the whole official-source fetch.
- After deterministic contact hints and bounded official contact-page fetching produce no usable email, allow at most one configured extraction-model call per account. The model may identify only an email and exact supporting excerpt visible in the fetched source; TypeScript rejects anything not present in that evidence.
...
- Evaluate `PUBLIC_EMAIL` after contact enrichment. Email-required missions reserve up to one focused search and three contact pages per candidate account before market search/source fetching, so the market phase cannot consume the entire contact budget. Accounts without a verified public email remain in audit data with `CONTACT_REQUIREMENT_NOT_MET` and are excluded from the email-required export.
