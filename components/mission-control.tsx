...
type DossierResponse = { id: string; status: string; discoveryStage: string; warnings?: Array<{ code: string; message: string }>; errors?: Array<{ code: string; message: string }>; mission: { name: string; owner: string; productFocus: string }; accounts: DossierAccount[]; review: { id: string; status: string; decision: { action?: string; note?: string } | null } | null };
...
export function MissionControl() {
  ...
  async function launchMission(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      setLaunchStatus("Searching DuckDuckGo and fetching official sources…");
      const response = await fetch("/api/missions/discover", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ name: missionName, geographies: [geography], accountCategories: ["TICKETED_EVENT_PROMOTER"], buyerRoles: [buyerRole], limits: { maxCandidateAccounts: 5, maxSearches: 12, maxPages: 20, maxModelCalls: 20, maxCostUsd: 2 } }) });
      const payload = await response.json() as LaunchResponse;
      if (!response.ok || !payload.missionRunId) throw new Error(payload.error?.message ?? "The mission could not be launched.");
      setRunId(payload.missionRunId); setLaunchStatus("Research complete. Loading persisted dossier…");
      const dossierResponse = await fetch(`/api/runs/${encodeURIComponent(payload.missionRunId)}`);
      const dossierPayload = await dossierResponse.json() as DossierResponse & { error?: { message?: string } };
      if (!dossierResponse.ok) throw new Error(dossierPayload.error?.message ?? "The persisted dossier could not be loaded.");
      setDossier(dossierPayload); setLaunchStatus(`Ready for review: ${dossierPayload.accounts.length} account${dossierPayload.accounts.length === 1 ? "" : "s"}.`);
  ...
  async function recordResearchGap() {
    ...
    try {
      ...
      setResearchGap(""); await loadDossier();
  ...
  return <main className="scout-grid min-h-screen overflow-hidden bg-[#0a0d12] px-5 py-6 text-[#f5f7fa] sm:px-8 lg:px-12"><div className="mx-auto max-w-7xl">
    ...
    <section className="grid gap-5 py-8 lg:grid-cols-[1.35fr_0.65fr]"><div className="rounded-3xl border border-white/10 bg-[#11161e]/90 p-6 shadow-2xl sm:p-8"><div className="mb-10 flex items-start justify-between gap-4"><div><p className="mb-2 text-xs font-bold uppercase tracking-[0.2em] text-white/45">Sales Mission Control</p><h2 className="text-2xl font-bold">Find the next credible conversation.</h2><p className="mt-3 max-w-xl leading-7 text-white/55">Load a persisted mission run, inspect its evidence-backed dossier and record Nick&apos;s review decision.</p></div><div className="hidden rounded-2xl border border-[#36d399]/25 bg-[#36d399]/10 px-3 py-2 text-right sm:block"><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-[#36d399]">System state</p><p className="mt-1 text-sm font-semibold text-white">Discovery ready</p></div></div>
      ...
      <div className="mt-10 border-t border-white/10 pt-8"><p className="text-xs font-bold uppercase tracking-[0.2em] text-[#f5c542]">Launch bounded mission</p><p className="mt-2 text-sm leading-6 text-white/50">Runs up to five candidate accounts through DuckDuckGo, safe fetch, extraction, verification and review persistence.</p><form className="mt-4 grid gap-3 sm:grid-cols-3" onSubmit={launchMission}><label className="text-xs text-white/50">Mission name<input value={missionName} onChange={(event) => setMissionName(event.target.value)} className="mt-1 w-full rounded-xl border border-white/15 bg-black/20 px-3 py-2 text-sm text-white" required /></label><label className="text-xs text-white/50">Geography<input value={geography} onChange={(event) => setGeography(event.target.value)} className="mt-1 w-full rounded-xl border border-white/15 bg-black/20 px-3 py-2 text-sm text-white" required /></label><label className="text-xs text-white/50">Buyer role<input value={buyerRole} onChange={(event) => setBuyerRole(event.target.value)} className="mt-1 w-full rounded-xl border border-white/15 bg-black/20 px-3 py-2 text-sm text-white" required /></label><button type="submit" disabled={isLaunching} className="rounded-full border border-[#36d399]/40 px-5 py-3 text-sm font-bold text-[#b7f5dd] disabled:opacity-50 sm:col-span-3">{isLaunching ? "Mission running…" : "Launch mission"}</button></form>{launchStatus ? <p className="mt-3 text-sm text-[#b7f5dd]" role="status">{launchStatus}</p> : null}</div>
      ...
      {dossier ? <DossierView dossier={dossier} isDeciding={isDeciding} isDrafting={isDrafting} isRecordingGap={isRecordingGap} researchGap={researchGap} onResearchGapChange={setResearchGap} onResearchGap={recordResearchGap} onDecision={decideReview} onDraft={draftFirstMove} /> : null}</div>
